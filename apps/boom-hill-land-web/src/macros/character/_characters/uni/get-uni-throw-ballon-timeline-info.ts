...
export function getUniThrowBallonTimelineInfo(props: GetTimelineInfoProps): GifMakerV2TimelineInfo {
  const { isRandomSpeed = false, loopCount } = props;

  return {
    timelineName: "uni-throw-ballon",
    timelines: [
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/uni/throw-ballon/throw-ballon-001.png`, // 뒷 모습
      ...
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/uni/throw-ballon/throw-ballon-002.png`, // 뒷 모습인 상태에서 던지려는 포즈
        durationMs: isRandomSpeed
          ? getRandomNumber({
              min: CHARACTER_ANIMATION_THROW_BALLON_RANDOM_SPEED_RANGE.MIN,
              max: CHARACTER_ANIMATION_THROW_BALLON_RANDOM_SPEED_RANGE.MAX,
      ...
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/uni/throw-ballon/throw-ballon-003.png`, // 뒷 모습인 상태에서 던진 후 모습
        durationMs: isRandomSpeed
          ? getRandomNumber({
              min: CHARACTER_ANIMATION_THROW_BALLON_RANDOM_SPEED_RANGE.MIN,
              max: CHARACTER_ANIMATION_THROW_BALLON_RANDOM_SPEED_RANGE.MAX,
