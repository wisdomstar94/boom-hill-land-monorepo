...
export function getEddiWakeUpTimelineInfo(props: GetTimelineInfoProps): GifMakerV2TimelineInfo {
  const { isRandomSpeed = false, loopCount } = props;

  return {
    timelineName: "eddi-wake-up",
    timelines: [
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/eddi/wake-up/wake-up-001.png`,
      ...
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/eddi/wake-up/wake-up-002.png`,
        durationMs: isRandomSpeed
          ? getRandomNumber({
              min: CHARACTER_ANIMATION_WAKE_UP_RANDOM_SPEED_RANGE.MIN,
              max: CHARACTER_ANIMATION_WAKE_UP_RANDOM_SPEED_RANGE.MAX,
