...
export function getMosRunTimelineInfo(props: GetTimelineInfoProps): GifMakerV2TimelineInfo {
  const { loopCount } = props;
  return {
    timelineName: "mos-run",
    timelines: [
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/mos/run/run-001.png`, // 정면 응시하는 모습
        durationMs: CHARACTER_ANIMATION_RUN_DEFAULT_DURATIONS["001"],
      },
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/mos/run/run-002.png`, // 고개 위로 올리고 눈 뜬 모습
        durationMs: CHARACTER_ANIMATION_RUN_DEFAULT_DURATIONS["002"],
      },
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/mos/run/run-003.png`, // 고개 위로 올리고 눈 감은 모습
