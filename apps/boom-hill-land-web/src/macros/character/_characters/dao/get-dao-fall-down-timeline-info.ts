...
export function getDaoFallDownTimelineInfo(props: GetTimelineInfoProps): GifMakerV2TimelineInfo {
  const { isRandomSpeed = false, loopCount } = props;

  return {
    timelineName: "dao-fall-down",
    timelines: [
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/dao/fall-down/fall-down-001.png`,
      ...
      {
        imageUrl: `${BASE_PATH}/boom-hill-character/dao/fall-down/fall-down-002.png`,
        durationMs: isRandomSpeed
          ? getRandomNumber({
              min: CHARACTER_ANIMATION_FALL_DOWN_RANDOM_SPEED_RANGE.MIN,
              max: CHARACTER_ANIMATION_FALL_DOWN_RANDOM_SPEED_RANGE.MAX,
